...
export default function invoiceRoutes(supabaseClient) {
  const router = express.Router();

  // Invoice for Sessions
  const TAX_RATE = 0.0725; // 7.25% tax rate, can be adjusted as needed
  
   router.get("/getInvoiceTableData", async (req, res) => {
  ...
  router.post("/:id/reduceRemainingInvoiceBalance", async (req, res) => {
    const { id } = req.params;
