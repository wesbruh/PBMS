  ...
  router.get("/:id/details", async (req, res) => {
    try {
      ...
      const { data: session, error: sessionError } = await supabaseClient
        .from("Session")
        .select("id, notes, SessionType(name)")
        .eq("id", id)
        .single();
      if (sessionError) throw sessionError;

      // get questionnaire responses for session
      const { data: response, error: responsesError } = await supabaseClient
        .from("QuestionnaireResponse")
        .select("id, status, submitted_at, QuestionnaireTemplate(name)")
        .eq("session_id", id)
        .maybeSingle();
